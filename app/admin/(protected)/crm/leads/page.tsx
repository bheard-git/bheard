...
export default function AdminCrmLeadsPage() {
  const { rows: leads, loading, error, retry } = useAdminApiList<any>("/api/contact-leads?admin=true");
  ...
  return (
    <div className="space-y-6">
      ...
      {error ? <DbErrorState title="Could not load leads" message={error} onRetry={retry} /> : null}
      {!loading && !error ? (
