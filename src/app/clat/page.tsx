    ...
    <>
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(categoryBreadcrumbJsonLd("clat")),
        }}
      />
      <Container>
        <Breadcrumb
        ...
        <Container>
          ...
          <RevealGroup>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {CLAT_TEST_SERIES.map((item, index) => (
                <TestSeriesCard
                  key={item.id}
                  item={item}
        ...
        <Container>
          ...
          <RevealGroup>
            <div className="flex flex-col lg:flex-row gap-4 lg:gap-5 items-stretch">
              <ResultsStatsPanel
                stats={CLAT_RESULT_STATS}
                className="reveal-child reveal-delay-1"
              />
              <div className="flex-1 min-w-0">
                <Carousel>
                  {clatResults.map((topper, index) => (
                    <div
                      key={topper.id}
                      className={`snap-start shrink-0 reveal-child reveal-delay-${(index % 4) + 1}`}
                    >
      ...
      <section id="testimonials" className="section-spacing">
        <Container>
          ...
          <RevealGroup>
            <Carousel autoPlay>
              {clatTestimonials.map((item, index) => (
                <div
                  key={item.id}
                  className={`snap-start shrink-0 reveal-child reveal-delay-${(index % 4) + 1}`}
                >
